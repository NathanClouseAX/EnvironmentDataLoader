        ...
        if ($lastStatus -in 'Succeeded', 'PartiallySucceeded') {
            ...
            if (-not [string]::IsNullOrWhiteSpace($downloadUrl)) {
                ...
                if ($DownloadPath -ne '') {
                    ...
                    try {
                        # Suppress the PS progress bar for the duration of the download.
                        # Invoke-WebRequest renders progress on every packet by default,
                        # which makes large blob downloads orders of magnitude slower in PS5.1.
